...
### 関数 `main` (L229-351)
- **役割**: コマンドのメインエントリーポイント。引数をパースして `-tree`, `-depth`, `-sort`, `-ignore`, `-match`, `-f` などのオプションを抽出し、対象パスに対して `buildSizeMap` や `renderTree` を呼び出す。
- **引数**:
...
- **既存コードへの影響**:
  - `buildSizeMap` 内でパターンマッチング処理が実行され、マッチしないパスが `sizeMap` から除外されます。
  - `renderTree` のフィルタリングロジックが `options.showFiles` および `sizeMap` のキー判定に依存します。
- **外部環境への影響**:
  - 新たに `-f` / `/f` / `--files`（ファイル表示）と、`-match` / `--match`（パターン指定）のオプションが追加されます。